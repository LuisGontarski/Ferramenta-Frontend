...
const ProjetosDetalhes = () => {
  ...
  const [trabalhoRestanteSprint, setTrabalhoRestanteSprint] = useState<
    number | string
  >("..."); // Inicia com '...'
  ...
  async function fetchBurndown(sprintId?: string) {
    const sprintIdToUse =
      sprintId ||
      selectedSprint ||
      localStorage.getItem("sprint_selecionada_id");
  ...
  const calculateSummaryCards = (
    currentBurndownData: any[],
    currentSprintEndDate: string | null
  ) => {
    if (currentBurndownData.length > 0) {
      const totalPontosInicial = currentBurndownData[0]?.real || 0;
      const pontosRestantesAtuais =
        currentBurndownData[currentBurndownData.length - 1]?.real ?? 0;

      // Progresso Sprint (%)
      const progresso =
        totalPontosInicial > 0
          ? Math.round(
              ((totalPontosInicial - pontosRestantesAtuais) /
                totalPontosInicial) *
                100
            )
          : pontosRestantesAtuais === 0
          ? 100
          : 0; // Se não há pontos, progresso é 100% se restante for 0
  ...
  async function fetchTotalTarefas() {
    try {
      const res = await fetch(
        `${BASE_URL}/projects/${selectedSprint}/tasks/count`
      );
      if (!res.ok)
  ...
  async function fetchCycleTime() {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_URL}/projects/${selectedSprint}/cycle-time`
      );
      if (!res.ok) throw new Error(`Erro ${res.status}`);
      const data = await res.json();
      // Arredonda para duas casas decimais ANTES de setar o estado
      const cycleTimeValue =
        data.cycleTime !== null ? parseFloat(data.cycleTime.toFixed(2)) : null;
  ...
  useEffect(() => {
    if (selectedSprint) {
      // Garante que só busca se houver uma sprint selecionada
  ...
  useEffect(() => {
    async function fetchMetrics() {
      ...
      try {
        const res = await fetch(
          `${BASE_URL}/projects/${id}/sprint/${selectedSprint}/metrics`
        );
        ...
        console.error("Erro ao carregar métricas da sprint:", err);
        setLeadTime(null);
        setVelocidadeEquipe(null);
        setTaxaBugs(null);
      }
    ...
    if (errorBurndown) {
      return (
        <p className="error-message">
          Erro ao carregar Burndown: {errorBurndown}
        </p>
      ); // Adicione estilo CSS
    }
    if (burndownData.length === 0) {
      return (
        <p className="empty-message">
          Dados do Burndown indisponíveis para esta sprint.
        </p>
      ); // Adicione estilo CSS
    }
          ...
          <div className="container_dashboard">
            ...
            <div className="container_informacoes_projeto_detalhes">
              ...
              <div className="div_informacoes_projeto_detalhes">
                ...
                <h2 className="valor_metricas_detalhes_projetos">
                  {cycleTime !== null ? `${cycleTime} dias` : "N/A"}
                </h2>
                <h2 className="adicional_metricas_detalhes_projetos">
            ...
            <div className="div_metricas_separadas">
              <div className="div_informacoes_projeto_detalhes">
                <h2 className="titulo_metricas_detalhes_projetos">Lead Time</h2>
                <h2 className="valor_metricas_detalhes_projetos">
                  {leadTime !== null ? `${leadTime} dias` : "N/A"}
                </h2>
                <h2 className="adicional_metricas_detalhes_projetos">
                  tempo médio por tarefa
                </h2>
              ...
              <div className="div_informacoes_projeto_detalhes">
                ...
                <h2 className="valor_metricas_detalhes_projetos">
                  {velocidadeEquipe !== null
                    ? `${velocidadeEquipe} pts`
                    : "N/A"}
                </h2>
                <h2 className="adicional_metricas_detalhes_projetos">
            ...
            <div className="card_atualizacoes">
              ...
              {errorBurndown && !loadingBurndown && (
                <p>Erro ao carregar resumo: {errorBurndown}</p>
              )}
            </div>
